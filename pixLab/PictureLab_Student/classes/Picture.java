...
public class Picture extends SimplePicture 
{
  ...
  public void mirrorHorizontal()
  {
	  ...
	  for(int row = 0; row < length/2; row++)
	  {
		  ...
		  {
			  topPixels = pixels[row][col];
			  bottomPixels = pixels[length - 1 - row][col];
  ...
  {
	  //row = 195, 161-190; col = 292/237
	  int mirrorPointleft = 195;
	  Pixel topPixel = null;
	  Pixel bottomPixel = null;
	  Pixel[][] pixels = this.getPixels2D();
	  for(int row = 161; row < mirrorPointleft; row++)
	  {
		  for(int col = 104; col < 171; col++)
		  {
			  topPixel = pixels[row][col];
			  bottomPixel = pixels[mirrorPointleft - row + mirrorPointleft][col];
			  bottomPixel.setColor(topPixel.getColor());
		  }
	  }
	  for(int row = 172; row < mirrorPointleft; row++)
	  {
		  for(int col = 237; col < 292; col++)
		  {
			  topPixel = pixels[row][col];
			  bottomPixel = pixels[mirrorPointleft - row + mirrorPointleft][col];
			  bottomPixel.setColor(topPixel.getColor());
  ...
  public void newCopy(Picture fromPic, int toStartRow, int toStartCol,
 ...
 public void myCollage()
  {
	  Picture nike = new Picture("nike.jpg");
	  Picture kitten = new Picture("kitten2.jpg");
	  kitten.mirrorHorizontalBotToTop();
	  this.newCopy(nike,0,0,126,99,268,357);
	  this.newCopy(kitten,0,200,116,165,180,309);
	  
	  Picture onlyBlueNike = new Picture(nike);
	  onlyBlueNike.keepOnlyBlue();
	  this.newCopy(onlyBlueNike,0,400,300,431,385,550);
